...
## 5. Deploying to Hostinger (GitHub auto-deploy)

**One-time setup — do this in order:**

1. **Install WordPress first** on Hostinger (hPanel → WordPress → Install) so that `public_html/` has WordPress core before any deployment runs.

2. **Connect GitHub in Hostinger hPanel:**
   - Go to **hPanel → Advanced → Git**
   - Click **"Connect to GitHub"** and authorise your account
   - Select repository: `aravind-raj17/syseze`, branch: `main`
   - **Deploy path:** `/public_html/wp-content/themes/syseze` ← this is critical
   - Enable **Auto-deploy on push**
   - Click **Deploy**

3. **Activate the theme** — WordPress admin → **Appearance → Themes** → activate **SysEze**.

4. **Regenerate .htaccess** — WordPress admin → **Settings → Permalinks** → click **Save Changes** (even without changing anything). Required for clean URLs.

5. Run the one-time setup in section 2 above.

**Why this structure works:**
The repo root IS the theme folder. Hostinger deploys repo root → `/public_html/wp-content/themes/syseze/`. WordPress core files at `public_html/` are never touched by the deployment.

**From now on:** every `git push` to `main` auto-deploys only the theme files to the live site.
...
```
syseze/ (repo root = theme root → deployed to public_html/wp-content/themes/syseze/)
