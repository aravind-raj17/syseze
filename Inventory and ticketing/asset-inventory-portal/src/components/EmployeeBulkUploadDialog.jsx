...
import { parseEmployeeImportCSV } from '../lib/employeeImport';
import { downloadEmployeeTemplate } from '../lib/employeeExport';
import { createEmployee } from '../lib/employees';

export default function EmployeeBulkUploadDialog({ open, clientId, organizationName, changedBy, onClose, onImported }) {
  ...
  const handleFile = async (e) => {
    ...
    setParsed(parseEmployeeImportCSV(text));
  };
  ...
  const handleImport = async () => {
    ...
    try {
      for (const employee of parsed.valid) {
        await createEmployee(clientId, organizationName, employee, changedBy);
      }
      onImported(parsed.valid.length);
      reset();
      onClose();
    } catch {
      setImportError('Something went wrong importing employees. No further rows were created — check the ones already saved before retrying.');
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={handleClose}>
      ...
      >
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">Bulk upload employees</h3>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Upload a CSV in the template format to add multiple employees to {organizationName || 'this client'} at once.
          </p>
        </div>
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-700">
            ...
            {parsed.errors.length > 0 && (
              <ul className="max-h-32 overflow-y-auto text-xs text-red-600 dark:text-red-400">
                {parsed.errors.map((e) => (
                  <li key={e.row}>Row {e.row} ({e.name}): {e.problems.join(', ')}</li>
                ))}
              </ul>
            )}
          </div>
        ...
        <div className="mt-2 flex justify-end gap-2">
          ...
          >
            {importing ? 'Importing…' : `Import ${parsed?.valid.length ?? ''} employees`}
          </button>
        </div>
      </div>
    </div>
